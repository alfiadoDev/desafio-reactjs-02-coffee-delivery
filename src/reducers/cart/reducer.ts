...
export function cartReducer(state: CarState, action: Actions) {
  switch (action.type) {
    ...
    case ActionTypes.INCREMENT_ITEM_QUANTITY:
      return produce(state, (draft) => {
        ...
        if (itemToIncrement) itemToIncrement.quantity += 1
      })
    ...
    case ActionTypes.DECREMENT_ITEM_QUANTITY:
      return produce(state, (draft) => {
        ...
        if (itemToDecrement && itemToDecrement) itemToDecrement.quantity -= 1
      })
